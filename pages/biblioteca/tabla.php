...
?>
    <table id="datatableRows" class="data-table nowrap hover">
        <thead>
            <tr>
                <th class="text-muted text-small">URL</th>
        ...
        <tbody>
            <?php
            $cad = "SELECT *FROM SYSTEM_UPLOADS";
            $sql = $db->query($cad);
            if ($db->rows($sql) > 0) {
                foreach ($sql as $key) {
                    print <<<SILVER
                    <tr>
                        <td>
                            <img src="{$key['URL']}" class="img-fluid rounded mb-1 me-1 sh-9" alt="Responsive image">
                        </td>
                        <td>{$key['NOMBRE']}</td>
                        <td>{$key['FECHA']}</td>
                        <td>
                            <button class="btn btn-sm btn-icon btn-icon-start btn-outline-primary ms-1" type="button" onclick="copiarAlPortapapeles('{$key['URL']}')">
                                ...
                                <span class="d-none d-xxl-inline-block">Copiar</span>
                            </button>
                            <button class="btn btn-sm btn-icon btn-icon-start btn-outline-primary ms-1" type="button" onclick="borrar('{$key['ID']}')">
