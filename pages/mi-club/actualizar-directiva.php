...
// Usar __DIR__ para ruta absoluta
define('UPLOAD_DIR', __DIR__ . '/../../assets/uploads/directiva-clubes/');
 ...
 */
function procesar_foto_directiva($file, $id_club, $cargo) {
    ...
    // Crear nombre único: club_cargo_timestamp.ext
    $filename = 'club_' . $id_club . '_' . strtolower($cargo) . '_' . time() . '.' . $ext;
    ...
    // Generar URL completa correcta
    $url_relativa = '/cpanel/assets/uploads/directiva-clubes/' . $filename;
...
try {
    ...
    if ($id_directiva <= 0 || $id_club <= 0) {
        throw new Exception('Datos inválidos');
    }

    // Verificar que la directiva pertenece al club del usuario
    $check = $db->query("SELECT ID FROM VRE_DIRECTIVA_CLUBES WHERE ID = $id_directiva AND ID_CLUB = $id_club");
    ...
    if ($accion === 'agregar' || $accion === 'actualizar') {
        ...
        if (isset($_FILES['foto'])) {
            ...
            if ($accion === 'actualizar') {
                $result = $db->query("SELECT $foto_field FROM VRE_DIRECTIVA_CLUBES WHERE ID = $id_directiva");
            ...
            $foto_path = procesar_foto_directiva($_FILES['foto'], $id_club, $cargo);
        }
        ...
        if ($db->query($sql)) {
            $temp->registrar_auditoria('DIRECTIVA_CLUBES', 'EDITAR', "Cargo $cargo del club ID $id_club actualizado");
        ...
        if ($db->query($sql)) {
            $temp->registrar_auditoria('DIRECTIVA_CLUBES', 'ELIMINAR', "Cargo $cargo del club ID $id_club eliminado");
            $response['success'] = true;
