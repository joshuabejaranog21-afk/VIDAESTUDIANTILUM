    ...
    <div class="container">
        <nav aria-label="breadcrumb">
            <ol class="breadcrumb mb-2" style="--bs-breadcrumb-divider-color:rgba(255,255,255,.5);">
                ...
                <li class="breadcrumb-item active text-white" aria-current="page">Instalaciones</li>
            </ol>
        </nav>
        <h1 style="font-size:2.25rem;font-weight:800;margin-bottom:.5rem;">
            <i class="fas fa-building me-3"></i>Instalaciones
        </h1>
        <p style="opacity:.9;margin:0;">Conoce los espacios disponibles para ti en el campus</p>
    </div>
</div>
    ...
    <?php else: ?>
        ...
        <div class="mb-4">
            <input type="text" id="buscador" class="form-control form-control-lg"
                placeholder="🔍 Buscar instalación..." style="border-radius:.75rem;border:2px solid #e9ecef;max-width:400px;">
        </div>
                ...
                <div class="portal-card">
                    <div class="card-img-wrap">
                        <?php if (!empty($inst['IMAGEN_URL'])): ?>
                            <img src="<?php echo $siteURL . htmlspecialchars($inst['IMAGEN_URL']); ?>" alt="<?php echo htmlspecialchars($inst['NOMBRE']); ?>">
                        <?php else: ?>
                            <i class="fas fa-building"></i>
                        <?php endif; ?>
                    </div>
                    <div class="card-body">
                        <h5 class="card-title"><?php echo htmlspecialchars($inst['NOMBRE']); ?></h5>
                        <p class="card-text"><?php echo htmlspecialchars(mb_substr($inst['DESCRIPCION'] ?? '', 0, 100)); ?><?php echo mb_strlen($inst['DESCRIPCION'] ?? '') > 100 ? '…' : ''; ?></p>
                        ...
                        <?php if (!empty($inst['CAPACIDAD'])): ?>
                            <p class="mb-1" style="font-size:.8rem;color:#8392ab;">
                                <i class="fas fa-users me-1"></i>Capacidad: <?php echo htmlspecialchars($inst['CAPACIDAD']); ?>
                            </p>
                        <?php endif; ?>
                        <?php if (!empty($inst['HORARIO'])): ?>
                            <p class="mb-2" style="font-size:.8rem;color:#8392ab;">
                                <i class="fas fa-clock me-1"></i><?php echo htmlspecialchars($inst['HORARIO']); ?>
                            </p>
                        <?php endif; ?>
                        <a href="<?php echo $portalURL; ?>instalacion/<?php echo $inst['ID']; ?>" class="btn-portal d-inline-block text-decoration-none">
        ...
        <p class="text-muted mt-3" id="sinResultados" style="display:none;">No se encontraron instalaciones con ese nombre.</p>
    <?php endif; ?>
</main>
...
document.getElementById('buscador')?.addEventListener('input', function() {
    ...
    document.querySelectorAll('.instalacion-item').forEach(el => {
        el.style.display = el.dataset.nombre.includes(q) ? '' : 'none';
    });
    const visibles = [...document.querySelectorAll('.instalacion-item')].filter(e => e.style.display !== 'none');
